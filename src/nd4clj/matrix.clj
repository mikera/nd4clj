...
(defn- concat-along [is-vec ^INDArray old matrix dim]
  (if (= (alength (.shape old)) 2) ;nd4j concat does not work for indexes hi
    (Nd4j/concat (int dim)
                 #^"[Lorg.nd4j.linalg.api.ndarray.INDArray;" (into-array INDArray matrix))
    (let [iterator (iterate-indexes (.shape old))
          to-return (Nd4j/create (.shape old))]
      (doseq [idx (range (.length old))]
        (let [i1 (nth iterator idx)
              i2 (keep-indexed #(if (not= %1 dim) %2) i1)
              sliced-matrix (matrix (nth i1 dim))]
          (.putScalar ^INDArray to-return #^ints (int-array i1)
                      (.getDouble ^INDArray sliced-matrix (int-array i2)))))
      to-return)))
...
(deftype clj-INDArray [^INDArray a ^Boolean vector ^Boolean scalar ^Boolean empty]
  ...
  (mp/dimension-count [m dimension-number]
    (cond
      (and scalar (not= dimension-number 0))
      (throw (IllegalArgumentException. "Scalar has not dimension 0."))

      (and vector (not= dimension-number 0))
      (throw (IllegalArgumentException. "Vector has not dimension 0."))

      (neg? dimension-number)
      (throw (IllegalArgumentException. "Negative dimension."))

      (>= dimension-number (mp/dimensionality m))
      (throw (IllegalArgumentException. (str "Dimension number higher than "
                                             (dec (mp/dimensionality m))))))
  ...
  mp/PMatrixAdd
  (mp/matrix-add [m w]
    (wrap-matrix m (.add ^INDArray a
                         ^INDArray (.a ^clj-INDArray (mp/construct-matrix m w)))
                 vector scalar))
  (mp/matrix-sub [m w]
    (wrap-matrix m (.sub ^INDArray a
                         ^INDArray (.a ^clj-INDArray (mp/construct-matrix m w)))
                 vector scalar))
  ...
  mp/PMatrixEquality
  (mp/matrix-equals [m m2]
    (if (= (vec (mp/get-shape m)) (vec (mp/get-shape m2)))
      (let [b (.a ^clj-INDArray (mp/construct-matrix m m2))]
        (.equals ^INDArray a ^INDArray b))
      false))
  mp/PMatrixScaling
  (mp/scale [m b]
    (wrap-matrix m (.mul ^INDArray a ^Number (double b))
                 vector scalar))
  (mp/pre-scale [m b]
    (wrap-matrix m (.mul ^INDArray a ^Number (double b))
                 vector scalar))
  ...
  (mp/identity-matrix? [m]
    (let [h (.a m)]
      (and (square? a)
           (mp/diagonal? m)
           (let [diag (.a ^clj-INDArray (mp/main-diagonal m))]
             (= 1.0
                (.minNumber ^INDArray diag)
                (.maxNumber ^INDArray diag))))))
  (mp/zero-matrix? [m]
    (let [h (.a m)]
      (and (zero? (.minNumber ^INDArray h))
           (zero? (.maxNumber ^INDArray h)))))
  ...
  mp/PMatrixTypes
  (mp/diagonal? [m]
    (and (triangleUpper a (aget (.shape ^INDArray a) 0) 0 0)
         (triangleLower ^INDArray a (aget (.shape ^INDArray a) 0) 0 0)))
  ...
  (mp/orthogonal? [m eps]
    (mp/matrix-equals-epsilon
     (wrap-matrix m (.mmul (.transpose a) a) vector scalar)
     (wrap-matrix m (Nd4j/eye (aget (.shape a) 0)) vector scalar) eps))
  ...
  (mp/broadcast [m target-shape]
    (let [dim-prj (->> target-shape count range (drop 1) int-array)
          ...
          to-ret (.exec (Nd4j/getExecutioner) vbn (int-array dim-prj))]
      (wrap-matrix m to-ret (not (= 1 (count target-shape))) false)))
  mp/PBroadcastLike
  (mp/broadcast-like [m z] ;; TODO correct? shouldn't z be an ndarray?
  ...
  mp/PRotate
  (mp/rotate [m dim pos]
    (let [^INDArray matrix (.a m)
          dim-sz (-> matrix (.size dim))
          components (if (not (.vector m))
                       (map #(.slice ^INDArray matrix % (int dim)) (range dim-sz))
                       (map #(sub-matrix matrix % % 0 0) (range (.length matrix))))
          n-pos (mod pos (if (.vector m) (.length matrix) dim-sz))
          to-ret (Nd4j/create #^ints (.shape ^INDArray matrix))
          swapped (vec (concat (take-last (- (count components) n-pos) components)
                               (take n-pos components)))]
      (wrap-matrix m (concat-along (.vector m) matrix swapped dim)
                   vector scalar)))
  mp/PVectorView
  (mp/as-vector [m]
    (if (and (= (alength (.shape a)) 2)
             (or (.isColumnVector a) (.isRowVector a) vector))
      (if (.isRowVector a)
        (wrap-matrix m a true false)
        (wrap-matrix m (.reshape a (int-array [1 (max (vec (.shape a)))])) true false))
      (convert-mn a (vec (.asDouble (.data (.ravel a)))))))
  mp/PReshaping
  ...
  mp/PSameShape
  (mp/same-shape? [w r]
    (let [b (convert-mn w r)]
      (and (= (mp/get-shape w) (mp/get-shape b))
           (= empty (.empty ^clj-INDArray b))
           (= scalar (.scalar ^clj-INDArray b))
           (= vector (.vector ^clj-INDArray b)))))
  ...
  mp/PMatrixCloning
  (mp/clone [m]
    (wrap-matrix m (.dup a) vector scalar))
  mp/PExponent
  (mp/element-pow [m exponent]
    (let [result (Nd4j/create (.shape a))]
      (.exec (Nd4j/getExecutioner) (Pow. (.dup a) result exponent))
      (wrap-matrix m result vector scalar)))
  mp/PSquare
  ...
  ;; TODO this does not work for vectors
  (seq [m] (seq a))
;;;;;;;;;;;;;;;;;;;;;;
;  PTypeInfo
)
...
(defn- wrap-matrix
  ([^clj-INDArray _ ^INDArray mx ^Boolean vector ^Boolean scalar]
   (->clj-INDArray mx vector scalar false))
  ([^clj-INDArray _ ^INDArray mx ^Boolean vector ^Boolean scalar ^Boolean empty]
   (->clj-INDArray mx vector scalar empty)))
...
(defn- convert-mn [m data]
  (if  (instance? nd4clj.matrix.clj-INDArray data)
    ...
    (let [data-p (cond ;(instance? org.nd4j.linalg.api.ndarray.INDArray data)
                   ...
                   (if (instance? java.lang.Number (first data))
                     [data]
                     (clojure.walk/prewalk
                      #(if (instance? org.nd4j.linalg.api.ndarray.INDArray %)
                         (first (convert-to-nested-vectors %)) %)
                      data))
                   ...
                   :else
                   (let [pre (m/to-nested-vectors data)]
                     (cond (mp/is-scalar? pre)
                           [[pre]]
                           (mp/is-vector? pre) [pre] :else pre)))
          crr (Nd4j/create
...
(comment
  ...
  (first (first (seq (m/matrix :nd4j [[1 2] [3 2]]))))
